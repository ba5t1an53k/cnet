...
void neural_network::setup(int num_input, int num_hidden, int num_out)
{
	...
	this->_w0 = new Eigen::MatrixXd(num_hidden, num_input);
	*_w0 = Eigen::MatrixXd::Random(num_hidden, num_input);
	this->_w1 = new Eigen::MatrixXd(num_out, num_hidden);
	*_w1 = Eigen::MatrixXd::Random(num_out, num_hidden);
	this->_b0 = 0;
	this->_b1 = 0;
...
void neural_network::train(Eigen::MatrixXd & sample_matrix, Eigen::MatrixXd & target_matrix, double alpha, double max_error_deviation, long max_epochs, double * error)
{
	...
	do {
		...
		if (loss < max_error_deviation || num_epochs == max_epochs)
		{
			is_training = false;
		}
		last_error = loss;
	...
	std::cout << "\nINFO:: ------------------------------------------------" << std::endl;
	std::cout << "INFO:: Prediction results:\n" << std::endl;
	for (size_t i = 0; i < sample_matrix.rows(); i++)
	{
		Eigen::MatrixXd sample = sample_matrix.row(i).transpose();
		predict(sample, &current_output);
		std::cout << "INFO: " << sample_matrix.row(i) << " | " <<  *current_output << " (" << target_matrix.row(i) << ")" << std::endl;
	}
	std::cout << "\nINFO:: ------------------------------------------------" << std::endl;
	*error = last_error;
}
